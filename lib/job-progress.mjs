...
import { mapMarkdownProse, splitStreamingMarkdown } from './markdown-code.mjs';
...
export function createProgressUpdateBuffer({
  ...
  return {
    ...
    drain({ final = true } = {}) {
      ...
      const ready = [];
      for (const [index, update] of selected.entries()) {
        // A different worker/type or a non-append update ends the previous
        // stream. Only the newest item can receive more deltas after this drain.
        const complete = final || index < selected.length - 1;
        const controlStart = complete ? update.text.length : pendingProgressControlStart(update.text);
        const split = splitStreamingMarkdown(update.text.slice(0, controlStart), {
          final: complete,
          streaming: update.append,
        });
        if (split.ready) ready.push({ ...update, text: split.ready });
        const pending = split.pending + update.text.slice(controlStart);
        if (pending) items.push({ ...update, text: pending });
      }
      return formatProgressUpdates(ready, { verbose });
    },
    hasUpdates() {
...
export function createProgressUpdateForwarder({
  ...
  const schedule = () => {
    ...
    timer = setTimeoutFn(() => {
      timer = null;
      return flush({ final: false }).catch((error) => {
  ...
  const flush = async ({ final = true } = {}) => {
    ...
    if (inFlight) {
      await activeFlush;
      return progressBuffer.hasUpdates() ? flush({ final }) : false;
    }
    const details = progressBuffer.drain({ final });
...
function normalizeProgressUpdate(update) {
  ...
  // Deltas are not standalone prose. Cleaning/trimming here loses indentation,
  // blank lines, spaces between tokens, and even CRLF split across two deltas.
  const text = String(update?.text ?? '').replace(/\u0000/g, '');
  if (!text || (!append && !text.trim())) return null;
  return {
    type,
    worker,
    append,
...
function formatProgressUpdates(updates, { verbose = false } = {}) {
  ...
  return mapMarkdownProse(
    cleanOutboundText(stripBridgeControlBlocksFromProgress(lines.join('\n'))),
    (prose) => prose.replace(/\n{2,}/g, '\n'),
  );
}
...
function pendingProgressControlStart(text) {
  const names = ['restart_service', 'wait_for_user', 'maintenance_issue_result', 'daily_maintenance_result'];
  const pattern = /<bridge_(restart_service|wait_for_user|maintenance_issue_result|daily_maintenance_result)>/gi;
  for (const match of text.matchAll(pattern)) {
    if (!text.toLowerCase().includes(`</bridge_${match[1].toLowerCase()}>`, match.index + match[0].length)) {
      return match.index;
    }
  }
  const start = text.lastIndexOf('<');
  const tail = text.slice(start).toLowerCase();
  return start !== -1 && names.some((name) => `<bridge_${name}>`.startsWith(tail))
    ? start
    : text.length;
}
